...
trait RecursiveCancelableBooking {    
    ...
    public function cancelBooking($customer){
        ...
        if($config->allow_cancellation == 1){
            ...
            BookableBooking::where('bookable_recurring_booking_id', $this->id)
                            ->update([
                                'cancelled_at' => Carbon::now()->toDateTimeString(),
                                'cancellable_type' => $customer->bookableGetClassName(),
                                'cancellable_id' => $customer->id
                            ]);
